...
public class GpuFifoScheduler extends
    AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements
    Configurable {

  private static final Log LOG = LogFactory.getLog(GpuFifoScheduler.class);
  ...
  private final ResourceCalculator gpuResourceCalculator = new GpuResourceCalculator();
  ...
  public GpuFifoScheduler() {
    super(GpuFifoScheduler.class.getName());
  }
  ...
  private synchronized void initScheduler(Configuration conf) {
    ...
    // The minimum gpu allocation is zero
  ...
  // This method only pulls the containers already allocated in Scheduler.nodeUpdate
  @Override
  public Allocation allocate(
    ...
    // Sanity check
    // This step is only needed for memory
  ...
  /*
   * This is called when client called submitApplication
   */
  @VisibleForTesting
  ...
  private void assignContainers(FiCaSchedulerNode node) {
    ...
    for (Map.Entry<ApplicationId, SchedulerApplication<FiCaSchedulerApp>> e : applications
      ...
      // Done
      // If available resource in node is less than minimum allocation, we could
      // stop allocate resource from this node
      if (Resources.lessThan(resourceCalculator, clusterResource,
          node.getAvailableResource(), minimumAllocation) ||
          Resources.lessThan(gpuResourceCalculator, clusterResource,
              node.getAvailableResource(), minimumAllocation)) {
        break;
      }
    }
  ...
  private int assignContainer(FiCaSchedulerNode node, FiCaSchedulerApp application,
    ...
    int availableContainers = Math.min(availableMemorySlots, availableGpuSlots);
...
//        node.getAvailableResource().getMemory() / capability.getMemory(); // TODO: A buggy
  ...
  // Allocate resource on this node to applications pending for resources
